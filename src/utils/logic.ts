...
export const stepBoard = (board: Board): Board => {
    ...
    for(let i = 0; i < BOARD_LENGTH; ++i){
      ...
      for(let j = 0; j < BOARD_LENGTH; ++j){
        const val = board[i][j];
        const count = checkNeibhourCount(board, i, j);
        if(val === 0){
          if(count === 3){
            row.push(1);
          }
...
export const checkNeibhourCount = (board: Board, x: number, y: number): number => {
  let count = 0;
  ...
  const checkList = [
    [up, left], [up, y], [up, right],
    [x, left], [x, right],
    [down, left], [down, y], [down, right],
];
...
export const flipCell = (board: Board, i: number, j: number): Board => {
     ...
     */
    // Step 3 BEGIN]
    const newBoard = getBlankBoard();
    for(let s = 0; s < BOARD_LENGTH; ++s){
      for(let t = 0; t < BOARD_LENGTH; ++t){
        newBoard[s][t] = board[s][t];
      }
    }
